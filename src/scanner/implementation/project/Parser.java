package scanner.implementation.project;
import java.util.*;
import java.io.*;
public class Parser {
	public String tree1;
	/*
	 * Implementing the Scanner using two methods 1. parse 2. printParseTree Print the
	 * intermediate states of the parsing process, including the intermediate
	 * states of the parse tree. If the
	...
	public void parse() {
		...
		hash2.put("Stack", 1);
		hash2.put("input tokens", 2);
		hash2.put("action lookup", 3);
		hash2.put("action value", 4);
		hash2.put("valueofLHS", 5);
		hash2.put("lengthofRHS", 6);
		hash2.put("tempstack", 7);
		hash2.put("gotolookup", 8);
		hash2.put("gotovalue", 9);
		hash2.put("stackaction", 10);
		...
		try {
			...
			for (int j = 0; j < s2.length;) {
				System.out.println("");
				int x = Integer.parseInt(hash.get(s2[j]).toString());
				...
				if (z == 'S') {
					int m = Integer.parseInt(lookUp.replaceAll("[^0-9]", "").toString());
					String a = s2[j] + m;
